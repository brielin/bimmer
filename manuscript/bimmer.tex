...
an SE \todo{is SE defined somewhere?} below $0.005$ were given the same weight.  We chose a target stability of $0.025$ which gave reliable results across our various simulations \todo{reference appropriate section}. This resulted in a regularization 
parameter $\lambda = 0.055$ \todo{is there any point putting this here? it's not interpretable}. The resulting DCE graph had $7,949$ edges, which we pruned to $2,826$ edges by removing DCEs with absolute value less than $0.1$. 
...
than the correlation of the TCE with $U$ ($r = 0.666 \pm 0.005$ vs $r = 0.949 \pm 0.004$, respectively \todo{cool!}).
...
out-degree to every other node in the network. The vast majority \todo{what proportion?} of these connections
are indirect and result in small effect sizes that do not reach
statistical significance as TCEs. Non-significant (at $5\%$ FDR) TCE entries have an average absolute shrunk TCE of $0.0036$ and a path length (in the CDE network) of $2.8$ \todo{would median path length make more sense?}, while
...
fraction of the total effect: there are often \todo{typically?} multiple paths between two nodes contribute to the TCE (Figure~\ref{figure6}c-d). As expected \todo{why is this expected?}, this finding is especially pronounced 
...
that these effects are mediated by an effect \todo{DCE?} of AFSI on BMI, which does not survive
...
To this end, we clustered the $102$ phenotypes with non-zero out-degree using their \todo{outgoing?} shrunk TCE estimates ($U_{i, :}$) as features.
...
and $177$ FDR $5\%$ significant TCEs. This network \todo{i.e. the heart disease network?} includes several well-studied phenomena
...
Interestingly, the network has an edge between \todo{from?}
leukocyte count and both systolic blood pressure and hypertension, but these do not survive correction
for multiple testing as TCEs ($p < 0.055$ and $p < 0.063$, respectively \todo{using $<$ is a bit odd here}). We consider this evidence of
a complex mechanism by which white blood cell traits effect heart disease risk via multiple causal pathways,
warranting further study. One interesting downstream effect of this \todo{i.e. the heart disease network?} sub-network is related
to choice of pain medication. We detect a positive causal effect \todo{TCE or DCE?} of high cholesterol on aspirin use
...
while increasing risk of lung cancer ($R = 0.013$, $p < 0.02$) \todo{is smoking considered? might except that to mediate this}. This lends causal support to
...
erythrocyte function \todo{I don't understand the point of this sentence}. In the latter network, erythrocyte count and haemoglobin concentration (HC)
...
for example plateletcrit \todo{what is this?} ($R = -0.13$, $p < 3 \times 10^{-11}$) and platelet count
...
\todo{can you put x-ticks at all the integers for a and b. I wonder if a histogram might be better for c and especially d so you can put a bin break at 1?}
...
Generally speaking \todo{I agree with discussing this but I don't think you should lead with it}, causal claims should be backed by thorough analysis resulting from
...
multivariate normal \todo{I don't really agree with this, it's just a different distributional assumption}. We found that for many classes of graphs,
...
complexity $O(??)$ compared to glassos \todo{$O(??)$}. In spite of these limitations,
...
statistics are much easier \todo{legally or practically or both?} to share and faster to work with when the primary
...
tend to be less polygenic with larger \todo{is this true? might depend on definition of polygenicity}, localized SNP effect sizes, which improves the efficiency
...
the SNP effects $\beta$ and the causal graph as fixed \todo{fixed as in fixed effects?}, and assume that the genotypes
$X$ are sampled uniformly \todo{do you mean iid?} from a population. For convenience, we assume that SNPs
...
Consider fitting the above model \todo{ref the equation} $Y = (X\beta + \gamma)(I-R)^{-1} + \epsilon$
using two-stage least-squares with $X$ as instruments\todo{remind us what gamma and epsilon are}. \todo{I would put this into first person or passive voice rather than the imperative}For now,
...
product. The TCE matrix is then\todo{give the Y= expression or at least ref eq 1 to show where this is coming from}
\begin{align*}
...
Intuitively\todo{explain this or maybe better still use a figure}, non-pleitropic SNPs will obey the relationship
...
$\hat{\beta}_{est}, \hat{s}_{est}$\todo{don't introduce these if you're not going to use them}. Next, for each
phenotype $i$ we identify the set of SNPs marginally associated at p-value threshold $p$.
Call this set $I_i = \{k: \hat{p}_{k, i} < p\}$\todo{this is on the discovery set?}. For every SNP $k \in I_i$ and
...
$w^{i,j}_k = \frac{|\hat{\beta}_{k, i}| - |\hat{\beta}_{k, j}|}
  {\sqrt{\hat{s}_{k,j}^2 (\hat{s}^2_{k, i} + \hat{s}^2_{k, j})}}$\todo{this is on the estimation set?}. This corresponds to the Welch test statistic for the means of two samples (with unequal variances) being different~\cite{Welch1947}.
...
solving~\eqref{r_cde} as finding a sparse matrix inverse \todo{only if the underlying R is actually sparse - I think you need to say that's a (very reasonably) assumption we're making}. Let $A$ be an arbitrary $D\times D$
...
we use the bigconjugate \todo{big not bi?} gradient stabilized method implemented
...
\todo{add an introductory sentence saying we use an adaptive of stability selection} Let $\phi_{\lambda}$ be a $D\times D$ matrix where entry $i, j$ is the probability that each edge
$i,j$ is included in the graph for regularization parameter $\lambda$ \todo{remind use which equation lambda is being used in}. Our goal is to
...
probability $1-p$. Let $V_{\lambda}^k = f(A, W^k, \lambda)$ \todo{what is the point of introducing f? you don't use it anyway} be
