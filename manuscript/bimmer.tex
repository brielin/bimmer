...
owing to factors such as unmeasured confounding and reverse causation (CITE?). % https://genomebiology.biomedcentral.com/articles/10.1186/s13059-019-1700-9 and references 8-12 therein
...
Another approach, the latent causal variable (LCV) model, is able to detect genetic causality \todo{define GC?}
...
to learn the direction of the effect from the data. \todo{"citation required" or back off this claim some}
...
However, methods to enact this conversion are limited. \todo{maybe cite e.g. the paper Nasa is on + Audrey Fu's paper and say why they're limited?}
This raises another disadvantage of approaches such as LCV and gwas-pw \todo{did you say what gwas-pw is?}. Assuming that either
A causes B or B causes A, but not both, is equivalent to assuming
that the underlying causal network
lacks cycles, which are thought to be an important part of real biological networks (CITE ??). \todo{I don't know if you even need a citation here: homeostasis and positive/negative feedback are textbook material. Would be safest to find one or two though}.  
...
We apply our method to \todo{NNN} phenotypes from the UK Biobank, finding thousands
...
The resulting equation can be arrived at via at least two orthogonal derivations. \todo{include these in the supp?}
...
CDE matrix is sparse. Specifically, we seek matrices $U$ and $V$ such that $VU=I$, $U \approx R^{TCE}$ \todo{would it be better to set this up without $U$ and assume $V^{-1}$, then introduce the relaxation in the methods?}
...
by setting their weights to 0. \todo{connect eq 2 and 3: how do I get $R$ from $V$?, does 3 give exactly eq2 for $\lambda=0$ and invertible $R^{TCE}$?}
...
partial correlations is $P = D[\Sigma^{-1}] \Sigma^{-1} D[\Sigma^{-1}]$ \todo{where does this come from? $P = -D[\Sigma^{-1}]^{-.5} \Sigma^{-1} D[\Sigma^{-1}]^{-.5}$}. One of
the most common approaches to obtaining a robust estimate of $\Sigma^{-1}$, also called
the precision matrix, is the graphical lasso (glasso) \todo{CITE}. The glasso assumes the data come from a multivariate normal distribution with a sparse
...
For complete details see Section~\ref{methods}. \todo{too much detail for Results section?}
...
the effect of $A$ on $B$ \todo{assuming no horizontal pleiotropy?}. That is, the SNP must have its per-variance contribution to
$B$ reduced by the network. We use this intuition to construct a new
weighting scheme for Egger regression.
First, we select a $p$-value threshold $p$ \todo{I'd make this $p_t$ or something}. For every phenotype $i$, we
...
second, these $1,000$ SNPs are again shared, but with correlated pleiotropy \todo{define this. maybe with a fig even?}
...
assumption commonly employed in causal inference (CITE ??). \todo{Stephen might know a cite for this?}

\todo{stronger intro sentence for fig caption}
...
biological networks (CITE ??) \todo{something from Barabasi}. In each setting we calculated the precision,
...
harmonic mean of precision and recall (CITE ??) \todo{don't think you need to cite anything for this}, as a function of the
...
region of stability around $\hat{D} = 0.05$. \todo{maybe don't use D since you already used above}
...
missingness, instead returning a matrix of NA values \todo{more formal way to state this would be that 'the glasso algorithm diverged', do people know what that means?}
(Figure~\ref{figureS2}).
...
applied (CITE MR-bma). \todo{I would reverse the last two sentences and introduce elnet-Egger as an approximation to MR-BMA}
...
for example, master regulators would have very high out-degree but low in-degree (CITE ??). \todo{omnigenic v2}
...
Our approach also still suffers from the small-effect bias \todo{=weak instrument?}, generally underestimating
...
and the omnigenic model\todo{cite}. The effects of genetic variants can be used to find and orient edges
...
We view all of these applications as important avenues for future work. \todo{last sentence is repetitive from previous paragraph(s)}
