...
\begin{abstract}
Inference of directed biological networks from observational genomics \todo{this has the flavor of molecular/functional data} datasets is a crucial
...
estimate the effect of every trait on every other. Next, approximately invert the resulting matrix \todo{not sure about the imperative here but I don't hate it either}.
...
for treatment~\cite{Boyle2017,Liu2019,Wray2018}\todo{cite omni v2 as well?}.
...
are severely limited~\cite{Chandrasekaran2012,Frot2019a,Frot2019b,Stegle2011}\todo{do we need to say why they're limited?} and have no strategy
...
G = I - R^{-1} D[1 / R^{-1}]
\end{equation}
\todo{this equation is for true not estimated R, right? Need to clarify that}where $D$ is an operator that sets all off-diagonal elements to 0, and $/$
...
\todo{would it make more sense to put this paragraph directly after eq 1?}Some intuition for~\eqref{r_dce_main} can be gained by considering estimating a matrix of partial correlations for a set of observed variables.
...
genome-wide significant (GWS, $p \le 5\times 10^{-8}$) SNPs for a trait \todo{the exposure?} as instruments.
...
weighting scheme for Egger regression that reduces the influence of SNPs that have overly-strong effects on both traits and are therefore more likely to be acting through pleiotropy. 
First, we select a $p$-value threshold $p_t$. For every phenotype $i$, we
 ...
 the Welch test statistic is~\cite{Welch1947}\todo{still wonder if the equation should go into the methods section}
\begin{equation}
...
stability of the inferred graph. For the graphical lasso, we used StARS\todo{cite} to
...
Our final\todo{you have another final goal later} goal was to show that bi-directional Mendelian randomization
...
to find sparse effects via LASSO or elastic net regression (elnet-Egger) or a sophisticated approach such as MR-Bayesian model averaging (MR-BMA)~\cite{Zuber2020}.

First, we simulated summary
statistics for $50$ phenotypes with $1,000$ shared and $2,000$ private
causal effect SNPs per pair of phenotypes, and $125,000$ total SNPs. Each
...
Our approach also builds on recent MR literature. In particular, multi-variable MR methods are able to compute direct causal effects when there
...
recently proposed CAUSE~\cite{Morrison2020} cause model where observed SNPs effects
...
Like most MR methods our approach is potentially susceptible to the weak instrument bias, which will result in  underestimating
...
However, a key advantage of our approach (beyond handling asymmetric matrices by design) is that we are able to incorporate
...
There are ``weighted'' extensions to the graphical lasso~\cite{Li2015,Zuo2017}, however 
these weights represent prior knowledge (by changing the regularization) rather than statistical uncertainty (corresponding to a changed likelihood) as required for our application.
...
of iterations required to reach convergence, glasso roughly requires time
$O(\kappa D^3)$, whereas inspre requires time roughly $O(\kappa D^4)$ for $D$ phenotypes\todo{don't think it's standard to include kappa, that's assumed}. We suspect that
...
In this work we have begun to elucidate the connection between MR
...
into~\eqref{r_dce}. However, we only have access to the noisy estimate $\hat{R}$ \todo{you use R as the TCE estimate in the results, make this consistent ideally},
...
solving~\eqref{r_dce} as finding a sparse matrix inverse. Let $A$\todo{I would vote for just using Rhat to avoid more notation} be an arbitrary $D\times D$
...
estimate $\phi_{\lambda}$ (and thereby the graph instability measure $D_\lambda$) for many choices of $\lambda$ .
...
The instability measure $D_\lambda$\todo{D being the dimension and the instability is unfortunate. Maybe change to $\Delta$?} is estimated as~\cite{Liu2010}
\begin{equation}
