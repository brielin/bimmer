...
measurements of traits, biomarkers, and genetic variation, offer an unprecedented opportunity
to study biological networks. Mendelian randomization (MR) has received attention as a class
of methods for inferring causal effects in observational data that uses genetic variants
...
on another, which may be mediated by other factors. Biobanks include measurements 
of many potential mediators, in principle enabling the conversion of MR estimates into direct effects 
representing a causal network.
Here, we show that this can be accomplished by a flexible two stage procedure we call
\emph{bidirectional mediated Mendelian randomization} (bimmer). First, bimmer
estimates the effect of every trait on every other. Next, bimmer finds
a parsimonious network that explains these effects using direct and mediated causal paths.
...
offer an unprecedented opportunity to study biological networks
...
an effect in each direction, or gwas-pw~\cite{Pickrell2016}, which infers the effect
...
In principle, this enables the conversion of TCE estimates into direct causal effect (DCE)
...
on every other, then approximately invert that matrix to produce a causal graph.
...
statistics. The first set is for SNP selection and weight construction,
 and the second set is for estimating $R$.
...
\subsubsection*{inspre is competitive with glasso while handling missing entries and directed graphs}
...
Therefore, we sought to compare these two methods with data simulated
from an undirected graph with normally-distributed observations (the glasso model).
We used the \emph{huge}~\cite{Zhao2012} package to generate data
...
random graphs and inspre outperformed glasso on hub graphs. Again both methods
...
results even when the maximum missingness was high. On the other
...
elnet-Egger, but they both performed poorly compared to bimmer.
...
elnet-Egger consistently had poor performance (Figure~\ref{figure4}d-i, Figure~S5).
 ...
 does not have enough variance to have nodes that consistently pull edges towards
...
the Neale lab, who corrected for age, age\textsuperscript{2} and 20 principal components
of the genotype matrix~\cite{NealeUKBB}. For ease of interpretation,
...
between two nodes that contribute to the TCE (Figure~\ref{figure6}c-d).
 This finding is especially pronounced 
when looking at all connections, where the median percentage of TCE explained
by shortest path is $0.35\%$ (Figure~\ref{figure6}c). For FDR $5\%$-significant connections,
the median percentage of TCE explained
...
phenotypes such as ``usual walking pace'' ($p < 2 \times 10^{-5}$, $U = -0.022$),
...
in neuronal pathways linked to reward~\cite{Ndiaye2020}, or result from a latent effect of
early puberty~\cite{Hulanicka2007,Solorzano2010}.
It is important to note that our analysis naturally controls for confounding by
socioeconomic status (SES). MR itself accounts for non-genetic confounding
by SES, and bimmer does not infer that the genetic effect is either mediated
by or jointly caused by ``Townsend deprivation index at recruitment''.
...
bimmer also puts a bi-directed edge between between mean sphered cell volume
and mean reticulyte volume, as well as between
sitting height and ``predicted forced expiratory volume in one second''.
While this does not happen universally,
...
These findings lend additional support to the recent literature on BMI as the cause of
...
of whether phenome-scale causal inference, where the number of pairs of traits to be tested
...
cannot be coherently incorporated into the network, and enable interpretation of
...
represent apparent causal effects rather than correlations. Then, we find a sparse approximate inverse
...
For example, Frot \emph{et al.}~\cite{Frot2019a} combines a low-rank plus sparse decomposition
...
leads to poor performance in practice~\cite{Pachter}. Finally, Dahl \emph{et al.}~\cite{Dahl2013}
and Stegle \emph{et al.}~\cite{Stegle2011}
model a low rank component in the observed data to capture population structure and latent
gene expression factors, respectively. These methods require access to the primary data,
...
UK Biobank summary statistics are available from the Neal lab at 
\url{https://www.nealelab.is/uk-biobank}. Our full data analysis results are available
at \url{https://zenodo.org/record/3895125}.

\section{Code Availability}
All code used in the production of this manuscript is available at
\url{https://github.com/brielin/bimmer} and \url{https://github.com/brielin/inspre}.
