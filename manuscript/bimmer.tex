...
we are not aware of any methods that are able to infer directed biological networks
...
causal direct effects, with $R_{i, j}$ the DCE of phenotype $i$ on
...
shrunk TCE ($U$) and bimmer's inferred DCE. First, we clustered phenotypes by genetic correlation
...
standard error that are consequently
...
Many indirect effects in the DCE network act through central hub vertices
...
section. Access to the DCE network, rather than just the TCEs, allows us to both understand potentially
...
that these effects are mediated by a DCE of AFSI on BMI, which does not survive
...
While this almost certainly does not represent a literal causal effect, the network structure lends
insight into the results and may lend additional evidence to recent work
showing that BMI-associated loci are involved
in neuronal pathways linked to reward~\cite{Ndiaye2020}.
...
In the the heart-disease network, leukocyte count had the highest out-degree with $244$ DCEs
...
the causal graph can remove false-positives with significant TCE $p$-values that
cannot be incorporated into the network, and allows researchers to 
interpret surprising TCEs in the context of the network structure.
...
the underlying directed graph of DCE is sparse. We observe that in ~\eqref{r_cde}, $R$ is
...
at \url{https://www.dropbox.com/s/lgf54tmoqu4u8d5/all_data.csv?dl=0}.

\section{Acknowledgments}
BCB would like to thank Lior Pachter and Nicolas Bray for insightful discussion of the
...
BCB wrote the code, conduced analyses, and drafted the manuscript.
