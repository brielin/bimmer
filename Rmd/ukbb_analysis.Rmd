...
#load(save_snps)
tce_res_aps <- fit_tce(sumstats, selected_snps, mr_method = "aps",
                   min_instruments = 5, verbose = TRUE)
save(tce_res_aps, file = "/gpfs/commons/home/bbrown/ukbb_network/saved_rdata/gencor_tce_res_aps.Rdata")
```
...
tce_filt <- filter_tce(R_tce, SE_tce)
...
tce_filt <- filter_tce(tce_res$R_tce, tce_res$SE_tce)
R_tce <- tce_filt$R_tce
SE_tce <- tce_filt$SE_tce

weights <- make_weights(SE_tce)

cde_res_wide <- fit_inspre(R_tce, W = weights, verbose = 2, cv_folds = 0, ncores = 8, warm_start = FALSE)
