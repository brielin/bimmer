...
plot_tibble <- foreach(i = 1:niter, .combine = bind_rows) %do% {
  ...
  foreach(method = c("oracle", "naive", "test"), .combine = bind_rows) %do% {
    if(method == "oracle"){
      ...
      selected <- select_snps(sumstats_select, p_thresh = 1e-4, welch_thresh = NULL)
    } else if (method == "test"){
      selected <- select_snps(sumstats_select, p_thresh = 0.01, welch_thresh = 0.1)
    }
    foreach(mr_method = c("raps"), .combine = bind_rows) %do% {
