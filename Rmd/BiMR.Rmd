...
```{r}
make_summary_fwer_tibble <- function(plot_tibble){
  summary_tibble <- plot_tibble %>% group_by(method) %>% 
    mutate(fwer1 = mean(p1 < 0.05, na.rm=T), fwer2 = mean(p2 < 0.05, na.rm=T),
           se_fwer1 = se(p1 < 0.05, na.rm=T), se_fwer2 = se(p2 < 0.05, na.rm=T),
    ...
    distinct(method, mr_method, thresh, p_thresh, w_thresh, fwer1, fwer2,
             se_fwer1, se_fwer2, mae1, mae2, se_mae1, se_mae2, n1_mean, n2_mean,
             se_n1, se_n2, obs1, obs2, se_obs1, se_obs2)
  return(summary_tibble)
}

make_summary_power_tibble <- function(plot_tibble){
  summary_tibble <- plot_tibble %>% group_by(method, r1_sim, r2_sim) %>% 
    mutate(power1 = mean(p1 < 0.05, na.rm=T), power2 = mean(p2 < 0.05, na.rm=T),
           se_power1 = se(p1 < 0.05, na.rm=T), se_power2 = se(p2 < 0.05, na.rm=T),
           mae1 = mean(abs(r1 - r1_hat), na.rm=T), mae2 = mean(abs(r2 - r2_hat), na.rm=T),
           se_mae1 = se(abs(r1 - r1_hat), na.rm=T), se_mae2 = se(abs(r2 - r2_hat), na.rm=T),
           n1_mean = mean(n1), n2_mean = mean(n2), se_n1 = se(n1), se_n2 = se(n2),
           obs1 = mean(!is.na(r1_hat)), obs2 = mean(!is.na(r2_hat)),
    ...
    distinct(method, mr_method, thresh, p_thresh, w_thresh, r1_sim, r2_sim, r1, r2, power1, power2,
             se_power1, se_power2, mae1, mae2, se_mae1, se_mae2, n1_mean, n2_mean,
             se_n1, se_n2, obs1, obs2, se_obs1, se_obs2)
  return(summary_tibble)
}
