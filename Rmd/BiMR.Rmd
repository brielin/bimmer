...
```{r}
run_sim <- function(n_iter, N, D, M_total, M_s, M_p, prop_shared, rho, noise, r1, r2, n_cores){
  fix_R =  matrix(c(0, r2, r1, 0), nrow = 2, ncol = 2)
  doMC::registerDoMC(cores = n_cores)
  result <- foreach(i = 1:n_iter, .combine = bind_rows) %dopar% {
    ...
    select_res <- foreach(p_thresh = 0.5*0.1^seq(1, 7), .combine = bind_rows) %do% {
      ...
      welch_res <- foreach(w_thresh = c(0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0), .combine = bind_rows) %do% {
...
r1 = 0 # NULL
r2 = 0
plot_tibble_1 = run_sim(
  n_iter = n_iter, N = N, D = D, M_total = M_total, M_s = M_s, M_p = M_p,
  prop_shared = prop_shared, rho = rho, noise = noise, r1 = r1, r2 = r2,
  n_cores = n_cores)
...
plot_tibble_2 = run_sim(n_iter, N, D, M_total, M_s, M_p, prop_shared, rho, noise, r1, r2, n_cores)
...
plot_tibble_3 = run_sim(n_iter, N, D, M_total, M_s, M_p, prop_shared, rho, noise, r1, r2, n_cores)
...
noise = 0.8 # 20% heritability
r1_values <- c(0.01, 0.05, 0.11, 0.22, 0.35, 0.58) # seq(0.05, 0.8, 0.05)
r2 = 0

plot_tibble_4 <- foreach(r1 = r1_values, .combine = bind_rows) %do% {
  run_sim(n_iter, N, D, M_total, M_s, M_p, prop_shared, rho, noise, r1, r2, n_cores)
}
...
rho = 0 # No genetic correlation.
noise = 0.8 # 20% heritability
r1_values <-  c(0.01, 0.05, 0.11, 0.22, 0.35, 0.58)
r2 = 0
...
rho = 0 # No genetic correlation.
noise = 0.8 # 20% heritability
r1_values <- c(-0.58, -0.28, -0.1, 0.1, 0.28, 0.58)
r2_values <- c(-0.58, -0.28, -0.1, 0.1, 0.28, 0.58)
...
ggarrange(f1a, f1b, f1c, f1d, f1e, f1f, ncol = 3, left = "FWER (null) or Power (alt)")
```
...
ggarrange(f2a, f2b, f2c, f2d, ncol = 2, widths = c(1.05, 1))
```
