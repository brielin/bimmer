...
```{r}
run_methods <- function(N, D, M_total, M_s, M_p, prop_shared, rho, noise, graph, prob, g, v, orient){
  ...
  selected_bimmer <- select_snps(sumstats_select, p_thresh = 5e-5, welch_thresh = 0.001)
  selected_standard <- select_snps(sumstats_select, p_thresh = 5e-8, welch_thresh = NULL)
  tce_res <- fit_tce(sumstats_fit, selected_bimmer, mr_method = "egger")
  ...
  result <- foreach(method = methods, .combine = bind_rows) %do% {
    ...
    if(method == "exact"){
      ...
      cde_res <- fit_inspre(R_tce = tce_res$R_tce, W = weights, cv_folds = 10, verbose = 1, lambda_min_ratio = 0.05, nlambda = 50, warm_start = FALSE)
      R_hat <- cde_res$R_hat
      D_hat <- cde_res$D_hat
...
plot_tibble_1 <- foreach(iter = 1:n_iter, .combine = bind_rows, .inorder = FALSE) %dopar% {
  run_methods(N, D, M_total, M_s, M_p, prop_shared, rho, noise, graph, prob, g, v, orient)
}
