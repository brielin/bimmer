...
```{r warning=FALSE}
fit_exact_function <- function(sumstats_fit, snps_to_use, tce_res){
  R_cde_hat <- fit_exact(tce_res$R_tce)$R_hat
  D <- dim(R_cde_hat)[1]
  R_cde_hat <- array(R_cde_hat, dim = c(D, D, 1))
  return(R_cde_hat)
}

fit_inspre_best_function <- function(sumstats_fit, snps_to_use, tce_res){
  weights <- inspre::make_weights(tce_res$SE_tce)
  cde_res <- fit_inspre(R_tce = tce_res$R_tce, W = weights, verbose = 1, rho = 10, mu = 10, lambda_min_ratio = 0.05, nlambda = 50, warm_start = FALSE)
  return(cde_res$R_hat)
}

fit_inspre_mask_function <- function(sumstats_fit, snps_to_use, tce_res){
  weights <- inspre::make_weights(tce_res$SE_tce)
  cde_res <- fit_inspre(R_tce = tce_res$R_tce, W = weights, cv_folds = 10, verbose = 1, rho = 10, mu = 10, lambda_min_ratio = 0.05, nlambda = 50, warm_start = FALSE)
  selected_index <- which(cde_res$D_hat > 0.05)[1]
  if(is.na(selected_index)){
    selected_index <- 50
  }
  return(cde_res$R_hat[ , , selected_index, drop = FALSE])
}

fit_mvmr_function <- function(sumstats_fit, snps_to_use, tce_res){
  # For compatibility with map_dfr(array_tree(...)) later.
  R_hat <- fit_mvmr(sumstats_fit, snps_to_use)$R_hat
  D <- dim(R_hat)[1]
